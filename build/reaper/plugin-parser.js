...
// ===== Parsing helpers =====
function extractVendorAndDisplay(fullName) {
...
function buildSearchTokens(displayName, vendor) {
...
// ===== Cache parser =====
async function parsePluginCache() {
    const entries = [];
...
// ===== Core resolver =====
export async function resolvePluginName(intent) {
    ...
    const fullText = (e) => `${e.displayName} ${e.vendor}`.toLowerCase();
    // Score VST3 > VST > AU for tie-breaking
    const sortByType = (a, b) => typeScore(b.type) - typeScore(a.type);
    // ---- Priority 1: exact display name match ----
    const exactMatches = index.filter(e => e.displayName.toLowerCase() === intentLower ||
        e.exactName.toLowerCase() === intentLower).sort(sortByType);
    ...
    if (longestToken.length > 1) {
        const mediumMatches = index.filter(e => e.displayName.toLowerCase().includes(longestToken) ||
            e.vendor.toLowerCase().includes(longestToken)).sort(sortByType);
...
export class ReaperPluginParser {
     ...
     */
    async getAvailablePlugins(searchQuery, maxResults = 50, forceRefresh = false) {
        if (forceRefresh)
            pluginIndex = null;
        const index = await ensureIndex();
        let results = index;
        ...
        const source = index.length > 0 ? 'cache' : 'fallback';
        return {
...
function fullTextOf(e) {
    return `${e.displayName} ${e.vendor}`.toLowerCase();
}
//# sourceMappingURL=plugin-parser.js.map