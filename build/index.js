...
import { buildFxChain, executeFxChain } from './mixing-engine.js';
import { resolvePluginName } from './reaper/plugin-parser.js';
/**
 * Convert audio file to MP3 using FFmpeg
 * Reduces file size by ~90% for AI analysis
 */
async function convertToMp3(audioFilePath, bitrate = 128) {
 ...
 */
async function analyzeLoudness(audioFilePath) {
    return new Promise((resolve, reject) => {
        const args = [
            '-i', audioFilePath,
...
class TaskManager {
    ...
    async cleanupOldTasks(maxAgeMs = 3600000) {
        const now = Date.now();
...
class ReaperMCPServer {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                switch (name) {
                    ...
                    case 'set_track_name': {
                        const { trackIndex, trackName } = args;
                    ...
                    case 'set_track_volume': {
                        const { trackIndex, volumeDb } = args;
                    ...
                    case 'set_track_output': {
                        const { sourceTrackIndex, destTrackIndex } = args;
                    ...
                    case 'batch_set_track_send': {
                        const { sourceTrackIndices, destTrackIndex, volumeDb } = args;
                    ...
                    case 'batch_set_track_output': {
                        const { sourceTrackIndices, destTrackIndex } = args;
                    ...
                    case 'add_fx_to_track': {
                        const { trackIndex, fxName, vendor } = args;
                    ...
                    case 'set_fx_param': {
                        const { trackIndex, fxIndex, paramIndex, value } = args;
                    ...
                    case 'set_fx_param_normalized': {
                        const { trackIndex, fxIndex, paramIndex, normalizedValue } = args;
                    ...
                    case 'set_fx_enabled': {
                        const { trackIndex, fxIndex, enabled } = args;
                    ...
                    case 'tweak_fx_parameter': {
                        const { trackIndex, fxName, paramIndex, normalizedValue } = args;
                    ...
                    case 'manage_track_routing': {
                        const { action, sourceTrackIndex, destTrackIndex, enable, sendVolumeDb, sendPan } = args;
                    ...
                    case 'split_item': {
                        const { trackIndex, itemIndex, position } = args;
                    ...
                    case 'get_item_info': {
                        const { trackIndex, itemIndex } = args;
                    ...
                    case 'analyze_and_suggest_mix': {
                        return await this.handleAnalyzeAndSuggestMix(args);
                    }
                    case 'start_audio_analysis': {
                        return await this.handleStartAudioAnalysis(args);
                    }
    ...
    async handleStartAudioAnalysis(args) {
        const { trackId, trackIds, renderMode = 'solo', startTime = 0, endTime } = args;
        // Validate parameters
    ...
    async runAnalysisTask(taskId) {
        try {
            ...
            const renderResult = await this.client.isolateAndRender(effectiveTrackId, startTime, actualEndTime, renderMode, trackIds);
            const statusFilePath = renderResult.statusFile;
            const audioFilePath = renderResult.filePath;
...
请确保参数值是具体的、可执行的数值或设置。`;
    ...
    async handleGetAnalysisStatus(args) {
        const { taskId } = args;
        const task = this.taskManager.getTask(taskId);
        if (!task) {
            return {
    ...
    async handleAnalyzeAndSuggestMix(args) {
        const { trackIndex, trackType, autoApply = false, trackId, trackIds, renderMode = 'solo', startTime = 0, endTime, } = args;
        ...
        const projectInfo = await this.client.getProjectInfo();
        const projectLength = projectInfo.projectLength ?? 0;
        ...
        const existingBuses = [];
        for (let i = 0; i < projectInfo.trackCount; i++) {
            try {
        ...
        try {
            ...
            if (Array.isArray(fxList) && fxList.length > 0) {
                currentFxChain = fxList.map((fx) => fx.name).join(', ');
            }
        }
        ...
        while (!renderCompleted && pollAttempts < maxPollAttempts) {
            try {
                ...
                if (status.status === 'completed') {
                    renderCompleted = true;
                    renderFilePath = status.filePath || renderResult.filePath;
                }
                else if (status.status === 'failed') {
...
{"track_type":"vocal|guitar|bass|drums|piano|synth|full_mix","problems":[{"issue":"问题描述","evidence":"来自上方数据的依据","severity":"high|medium|low"}],"freq_problems":{"rumble":{"detected":false,"severity":0.0,"center_freq":60},"boomy":{"detected":false,"severity":0.0,"center_freq":150},"muddy":{"detected":false,"severity":0.0,"center_freq":300},"harsh":{"detected":false,"severity":0.0,"center_freq":3500},"thin":{"detected":false,"severity":0.0,"center_freq":200},"dull":{"detected":false,"severity":0.0,"center_freq":10000},"sibilance":{"detected":false,"severity":0.0,"center_freq":7500}},"dynamic_range":"wide|medium|narrow","noise_floor":"clean|some|heavy","needs_reverb":false,"needs_delay":false,"reverb_send_level":0.25,"delay_send_level":0.20}`;
        ...
        let diagnosis;
        try {
            diagnosis = parseAiResponse(aiResponse);
