...
function eqFreqNorm(freq_hz) {
    return Math.min(1, Math.max(0, freq_hz / 24000));
}
function eqGainNorm(gain_db) {
...
// ===== Rule engine =====
export function buildFxChain(analysis) {
    const chain = [];
    ...
    const addBand = (arr, p, isCut, label) => {
    ...
    const eqCuts = [];
    const cutLabels = [];
    if (fp.rumble.detected) {
        addBand(eqCuts, fp.rumble, true, 'HP rumble');
        cutLabels.push('rumble');
    }
    if (fp.boomy.detected) {
        addBand(eqCuts, fp.boomy, true, 'Cut boomy');
        cutLabels.push('boomy');
    }
    if (fp.muddy.detected) {
        addBand(eqCuts, fp.muddy, true, 'Cut muddy');
        cutLabels.push('muddy');
    }
    if (fp.harsh.detected) {
        addBand(eqCuts, fp.harsh, true, 'Cut harsh');
        cutLabels.push('harsh');
    }
    if (eqCuts.length > 0) {
        chain.push({
            type: 'fx',
    ...
    const boostLabels = [];
    if (fp.thin.detected) {
        addBand(eqBoosts, fp.thin, false, 'Boost thin');
        boostLabels.push('thin');
    }
    if (fp.dull.detected) {
        addBand(eqBoosts, fp.dull, false, 'Air boost dull');
        boostLabels.push('dull');
    }
...
// ===== Executor =====
export async function executeFxChain(trackIndex, chain, analysis, fileClient) {
    const applied = [];
    const errors = [];
    ...
    for (const op of chain) {
        try {
            if (op.type === 'fx') {
                ...
                let busTrackIndex = -1;
                const trackCount = projectInfo.trackCount ?? 0;
...
//# sourceMappingURL=mixing-engine.js.map